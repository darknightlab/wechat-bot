...
# install get_session_token
RUN cd /env && virtualenv -p python3 chatgpt-python && \
    cd / && . env/chatgpt-python/bin/activate && \
    cd /wechat-bot && pip install --no-cache-dir -r requirements.txt
