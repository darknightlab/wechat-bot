...
class ChatBot {
    ...
    smallModel;
    smallModelInstructions = `之前的对话是你和其他人的聊天内容，你的ID是${this.name}。你的任务是查看之前的所有对话，并给我一个json。json的格式必须为： {send: boolean,voice: boolean}。如果你觉得你应该继续回消息，则send为true，否则为false；如果你希望发送语音消息，则voice为true。`;
    mainModel;
    mainModelInstructionsPrefix = `
...
你只需要回复普通格式的消息，我会用程序帮你转换成正确格式。`;
    ...
    newEmptyMessage(conversationType) {
        switch (conversationType) {
            case "contact":
                return { role: "system", content: [{ type: "text", text: this.config.chatbot.chatllm.contactRole }] };
            case "room":
                return { role: "system", content: [{ type: "text", text: this.mainModelInstructionsPrefix + this.config.chatbot.chatllm.roomRole }] };
            default:
    ...
    async shouldReplyBySmallModel(conversation) {
